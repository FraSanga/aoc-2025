defmodule Boxes do
  ...
  def connect(map, points) do
    ...
    do_connect(map, candidates, [], [])
    |> Enum.map(fn point -> String.split(point, ",") |> List.first() |> String.to_integer() end)
    |> Enum.product()
  end

  defp do_connect(map, candidates, visited, connected) do
    ...
    if length(new_visited) == @to_connect, do: [a,b], else: do_connect(new_map, new_candidates, new_visited, new_connected)
  end
  ...
  defp search_candidates(map, [point | points], candidates) do
    ...
    if next != nil do
      ...
      search_candidates(map, points, candidates)
    end
  end
  ...
  defp do_min_distance(_, [], connected, nil), do: {nil, nil, connected}
